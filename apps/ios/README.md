...
- Beta release uses canonical `it.differen.openclaw*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Release signing uses `Apple Distribution`; development builds keep `Apple Development`.
- CarPlay is excluded from Release until Apple confirms the entitlement for the Differen team. Debug keeps the existing entitlement for eligible development profiles.
