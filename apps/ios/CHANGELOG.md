...
- Fixed the chat unread badge staying on after reading a channel (build 14). The badge count only refreshed on a poll that runs every 20 seconds, so a channel you had just read kept its dot for up to half a minute. Opening a channel now clears its badge immediately, and the count returns from the server on the next refresh, so a message arriving in the meantime still shows up.
- Your device identity now survives signing out (build 14). Logging out discarded the device key, so the next sign-in registered a brand-new device and the list of your devices kept growing. The same phone is now recognized as the same device.
- Chat now opens on the last message instead of at the top of the history, on first open, after sending, and on every channel switch (build 12). The build 11 attempt moved the view before the messages existed, so it kept landing at the top.
- Fixed Iànua Realtime not working on first entry (build 13). Turning on voice processing, which happens once per app launch, changes the microphone bus from two channels to one, and the reconfiguration that follows stopped the audio engine about 100 ms after starting it had already reported success. The session therefore looked healthy and said it was listening while not a single microphone buffer ever arrived; entering a second time worked because voice processing was already on and nothing reconfigured. The audio graph is now rebuilt until the engine is genuinely running.
- The build 12 attempt at the same defect removed the realtime session opened in advance in the background. That was not the cause, since the defect survived it, but the removal is kept: the first entry now follows the same path as the second, which is the one that works (build 12).
