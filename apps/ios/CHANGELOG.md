...
- Fixed Iànua Realtime not working on first entry: the voice screen no longer takes over a session opened in advance in the background. Adopting that session was the only difference between the first entry, which did not work, and the second one, which did. Connecting now always takes the working path, about 2.5 seconds (build 12).
