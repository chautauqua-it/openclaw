...
- Chat now opens on the last message instead of at the top of the history, on first open and on every channel switch (build 11).
- Fixed Iànua Realtime failing to start on first entry: the voice screen no longer waits without a deadline for the session being prepared in the background, which could leave it on "connecting" for up to half a minute before the real attempt began. Leaving and re-entering used to be the only way through (build 11).
