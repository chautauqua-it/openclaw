...
if [[ -z "${TEAM_ID}" ]]; then
  echo "Company beta release requires an explicit IOS_DEVELOPMENT_TEAM or --team-id; automatic team selection is disabled." >&2
  exit 1
fi
...
OPENCLAW_APP_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}
OPENCLAW_SHARE_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}.share
OPENCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}.activitywidget
OPENCLAW_INTENTS_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}.intents
OPENCLAW_WATCH_APP_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}.watchkitapp
OPENCLAW_WATCH_EXTENSION_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}.watchkitapp.extension
OPENCLAW_WATCH_CONTROL_WIDGET_BUNDLE_ID = ${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}.watchkitapp.controlwidget
...
OPENCLAW_WATCH_CONTROL_WIDGET_PROFILE =
${PUSH_XCCONFIG_BLOCK}
EOF

IOS_DEVELOPMENT_TEAM="${TEAM_ID}" \
  IOS_BETA_BUNDLE_ID_BASE="${IOS_BETA_BUNDLE_ID_BASE:-it.differen.openclaw}" \
  bash "${ROOT_DIR}/scripts/ios-company-signing-audit.sh" "${BETA_XCCONFIG}"
